...
static void PlaceStructTick( string structName ) {
    ...
    if ( Cl.mouse0Down ) {
        if ( game.GetFirstPawnOnHex( Cl.mouseHex, out int z ) ) {
            ...
            Cl.SvCmd( $"sv_forced_spawn {structName} {Cellophane.FtoA( v.x )} {Cellophane.FtoA( v.y )}" );
        }
    }
...
static void PlaceSpawnZones_tck() {
    ...
    if ( Cl.mouse0Down ) {
        ...
        if ( zd.id != 0 && _zoneId == 0 ) {
            _zoneId = zd.id;
        } else {
            if ( _zoneId == 0 ) {
                for ( int i = 1; i < board.filter.zones.Length; i++ ) {
                    if ( board.filter.zones[i].polygon.Count == 0 ) {
                        _zoneId = i;
                        break;
                    }
                }
    ...
    if ( _zoneId != 0 ) {
        QGL.LatePrint( $"{_zoneId}", Cl.mousePosScreen - new Vector2( 0, 10 ), color: Color.white );
    }
